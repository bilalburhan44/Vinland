...
const DownloadTransactionPDF = ({ transaction, client }) => {
  ...
  const generatePDF = () => {
    ...
    // Add header text
    doc.setFontSize(20);
    doc.text("Vinland Kitchen", 180, 20, { align: "right" });
    ...
    doc.text(`Date: ${moment(transaction.date).tz("Asia/Baghdad").format("YYYY-MM-DD")}`, 150, 40, { align: "right" });
